// src/config.js
var DEFAULT_MARKET_ORIGIN = "https://xmarket.aidenhuang.com";

// src/content.js
var HOST_ATTRIBUTE = "data-signal-market-card";
var EMBED_VERSION_ATTRIBUTE = "data-xmarket-embed-version";
var EMBED_VERSION = "0.3.4";
var processedAnchors = /* @__PURE__ */ new WeakSet();
var processedArticleText = /* @__PURE__ */ new WeakMap();
var scanScheduled = false;
var configuredOrigin = DEFAULT_MARKET_ORIGIN;
var compactNumber = new Intl.NumberFormat("en-US", {
  notation: "compact",
  maximumFractionDigits: 1
});
var ISOLATED_INTERACTION_EVENTS = [
  "pointerdown",
  "pointerup",
  "mousedown",
  "mouseup",
...
function pageUsesDarkTheme() {
  ...
  for (const candidate of candidates) {
    ...
    const luminance = (match[0] * 299 + match[1] * 587 + match[2] * 114) / 1e3;
...
function renderMarket(shadow, market, marketOrigin, source, authState, positions = []) {
  ...
  const isOpen = market.status === "open" && new Date(market.closesAt) > /* @__PURE__ */ new Date();
  ...
  const accountLabel = user ? `Trading as ${user.xHandle ? `@${user.xHandle.replace(/^@/, "")}` : user.displayName}${authState.balance == null ? "" : ` \xB7 ${Number(authState.balance).toFixed(2)} credits`}` : authState?.configured ? "Connect your xmarket account to trade" : "Open the extension popup to connect Supabase";
  shadow.innerHTML = `
    ...
    <section class="card" aria-label="xmarket prediction market">
      ...
      <div class="trade">
        ...
        <div class="side-row">
          <button class="side yes selected" data-side="YES"><span>YES</span><strong data-yes-price>${Math.round(yesPrice * 100)}\xA2</strong></button>
          <button class="side no" data-side="NO"><span>NO</span><strong data-no-price>${Math.round((1 - yesPrice) * 100)}\xA2</strong></button>
        </div>
  ...
  const updateSummary = () => {
    ...
    if (actionButton) {
      ...
      const sellingAll = selectedMode === "sell" && owned[selectedSide] > 0 && Math.abs(selectedAmount - owned[selectedSide]) < 1e-6;
      actionButton.textContent = selectedMode === "buy" ? `Buy ${selectedSide} \xB7 $${selectedAmount}` : `${sellingAll ? "Sell all" : "Sell"} ${selectedSide} \xB7 ${selectedAmount.toFixed(2)}`;
    }
  };
  ...
  tradeButton?.addEventListener("click", async () => {
    ...
    try {
      ...
      shadow.querySelector("[data-yes-price]").textContent = `${Math.round(newYesPrice * 100)}\xA2`;
      shadow.querySelector("[data-no-price]").textContent = `${Math.round((1 - newYesPrice) * 100)}\xA2`;
      shadow.querySelector("[data-volume]").textContent = `$${compactNumber.format(Number(result.market.volume))} volume`;
      shadow.querySelector("[data-account-label]").textContent = orderMode === "buy" ? `Bought ${Number(result.shares_bought).toFixed(2)} ${orderSide} shares \xB7 ${Number(result.balance).toFixed(2)} credits left` : `Sold ${Number(result.shares_sold).toFixed(2)} ${orderSide} shares for ${Number(result.proceeds).toFixed(2)} credits`;
      updateSummary();
    } catch (error) {
...
async function start() {
  ...
  chrome.storage.onChanged.addListener((changes) => {
    if (changes.marketOrigin?.newValue) {
      ...
      processedAnchors = /* @__PURE__ */ new WeakSet();
      processedArticleText = /* @__PURE__ */ new WeakMap();
      scheduleScan();
    }
  });
}
