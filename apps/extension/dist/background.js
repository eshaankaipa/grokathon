...
var LEGACY_MARKET_ORIGINS = /* @__PURE__ */ new Set([
  "http://localhost:5175",
  "https://xpred.aidenhuang.com"
]);
function normalizeMarketOrigin(origin) {
  const normalized = typeof origin === "string" ? origin.trim().replace(/\/$/, "") : "";
  return !normalized || LEGACY_MARKET_ORIGINS.has(normalized) ? DEFAULT_MARKET_ORIGIN : normalized;
}
var DEFAULT_SUPABASE_URL = "https://fxoiyujqvfnclobriker.supabase.co";
var DEFAULT_SUPABASE_PUBLISHABLE_KEY = "sb_publishable_b3S00ffqP0mRMMjdDFBzOg_FPjNljlS";

// src/auth.js
var AUTH_STORAGE_KEY = "signal-extension-auth";
...
async function getAuthState() {
  ...
  if (data.session?.user) {
    const { data: profile, error: profileError } = await client.from("profiles").select("demo_balance").eq("id", data.session.user.id).single();
...
async function buyPosition({ marketSlug, outcome, amount, clientOrderId }) {
  const client = await getClient();
...
async function getMarketPositions(marketSlug) {
  const client = await getClient();
  const { data: sessionData, error: sessionError } = await client.auth.getSession();
  if (sessionError) throw sessionError;
  if (!sessionData.session) return [];
  const { data, error } = await client.from("positions").select("outcome,shares,average_price,markets!inner(slug)").eq("markets.slug", marketSlug).gt("shares", 0);
  if (error) throw error;
  return data || [];
}
async function sellPosition({ marketSlug, outcome, shares, clientOrderId }) {
  const client = await getClient();
  const { data: sessionData, error: sessionError } = await client.auth.getSession();
  if (sessionError) throw sessionError;
  if (!sessionData.session) throw new Error("Connect your xmarket account before trading.");
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  await chrome.storage.sync.set({
    ...
    supabasePublishableKey: values.supabasePublishableKey || DEFAULT_SUPABASE_PUBLISHABLE_KEY
  });
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.type === "SIGNAL_GET_MARKET") {
    fetchMarket(message.marketId).then((result) => sendResponse({ ok: true, ...result })).catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  ...
  if (message?.type === "SIGNAL_GET_AUTH") {
    getAuthState().then((auth) => sendResponse({ ok: true, ...auth })).catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  if (message?.type === "SIGNAL_GET_MARKET_POSITIONS") {
    getMarketPositions(message.marketSlug).then((positions) => sendResponse({ ok: true, positions })).catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  if (message?.type === "SIGNAL_CONNECT_WEBSITE") {
    connectWithWebsite().then((user) => sendResponse({ ok: true, user })).catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  if (message?.type === "SIGNAL_SIGN_IN_EMAIL") {
    signInWithEmail(message.email, message.password).then((user) => sendResponse({ ok: true, user })).catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
  if (message?.type === "SIGNAL_SIGN_UP_EMAIL") {
    signUpWithEmail(message.email, message.password).then((result) => sendResponse({ ok: true, ...result })).catch((error) => sendResponse({ ok: false, error: error.message }));
    return true;
  }
